...
export const Earnings = () => {
  ...
  const [activeTab, setActiveTab] = useState('pt'); // 'pt' | 'payslips'
  ...
  const loadSessions = async () => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from('pt_sessions')
        .select('*, coach:coach_id(full_name, email), member:member_id(full_name, email)')
