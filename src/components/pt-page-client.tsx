...
// Tap-to-WhatsApp reminder for a PT session, prefilled with the session date/time.
// Goes to the client (or the guardian for a kid). Sent from the coach's own WhatsApp.
function waPtReminderLink(s: {
  scheduled_at: string;
  member?: { full_name?: string | null; phone?: string | null } | null;
  package?: { guardian_name?: string | null; guardian_phone?: string | null } | null;
}): string {
  const raw = (s.member?.phone || s.package?.guardian_phone || "").replace(/\D/g, "");
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {isUpcoming && (s.member?.phone || s.package?.guardian_phone) && (
                        <a
                          href={waPtReminderLink(s)}
                          target="_blank"
                          rel="noopener noreferrer"
                          aria-label="WhatsApp session reminder"
                          className="w-9 h-9 flex items-center justify-center rounded-full bg-green-500/20 text-green-400 border border-green-500/30 hover:bg-green-500/30 transition-colors shrink-0"
                        >
