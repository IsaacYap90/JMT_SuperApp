  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-2">
        ...
        <label className={`px-4 py-2 min-h-[40px] bg-jai-card border border-jai-border text-sm text-jai-text rounded-full transition-colors flex items-center gap-1.5 cursor-pointer hover:text-white hover:border-jai-blue/40 ${uploading ? "opacity-50 pointer-events-none" : ""}`}>
      ...
      {earnings.length === 0 && !showForm && (
        <div className="bg-jai-card border border-jai-border rounded-xl p-6 text-center">
          <p className="text-jai-text">No earnings recorded yet. Tap + to add an entry.</p>
        </div>
      )}

      {/* Floating action button — primary "+" Add entry */}
      {!showForm && (
        <Fab
          ariaLabel="Add earning entry"
          onClick={() => {
            setEditingId(null);
            setForm({ date: new Date().toISOString().split("T")[0], type: "pt_weekly", amount: "", description: "" });
            setShowForm(true);
